...
if ( ! class_exists('Haru_Framework_Shortcode_Products_Top_Seller') ) {
    class Haru_Framework_Shortcode_Products_Top_Seller {
        function __construct() {
            add_shortcode( 'haru_products_top_seller', array($this, 'haru_products_top_seller_shortcode' ) );
            add_action( 'vc_before_init', array($this, 'haru_products_top_seller_vc_map') );
        }

        function haru_products_top_seller_shortcode($atts) {
            global $woocommerce;

            $atts  = vc_map_get_attributes( 'haru_products_top_seller', $atts );
            $title = $layout_type = $products_per_page = $columns = $rows = $autoplay = $slide_duration = $el_class = $css_animation = $duration = $delay = $haru_animation = $styles_animation = '';
            ...
            <div class="<?php echo esc_attr($haru_animation . ' ' . $styles_animation); ?>">
                <?php echo haru_get_template('products/top-seller.php', array('atts' => $atts), '', '' ); ?>
            </div>
        ...
        function haru_products_top_seller_vc_map() {
            vc_map(
                array(
                    'base'        => 'haru_products_top_seller',
                    'name'        => esc_html__( 'Haru Products Top Seller', 'haru-pangja' ),
                    'icon'        => 'fa fa-shopping-cart haru-vc-icon',
                    'category'    => HARU_PANGJA_CORE_SHORTCODE_CATEGORY,
                    'description' => esc_html__( 'Display products top seller.', 'haru-pangja' ),
                    'params'      => array(
                        array(
                            'param_name'  => 'title',
                            'type'        => 'textfield',
                            'heading'     => esc_html__( 'Title', 'haru-pangja' ),
                            'description' => esc_html__( 'Enter title of Product Top Seller.', 'haru-pangja' ),
                        ),
                        array(
                            'param_name'  => 'layout_type',
                            'type'        => 'dropdown',
                            'heading'     => esc_html__( 'Layout type', 'haru-pangja' ),
                            'description' => esc_html__( 'Choose layout type of products top seller', 'haru-pangja'),
                            'admin_label' => true,
                            'std'         => 'carousel',
                            'value'       => array(
                                esc_html__( 'Carousel (1 Column - Multi Rows)', 'haru-pangja' ) => 'carousel_1',
                                esc_html__( 'Grid (Multi Columns)', 'haru-pangja' )             => 'grid',
                            )
                        ),
                        array(
    ...
    new Haru_Framework_Shortcode_Products_Top_Seller();
}