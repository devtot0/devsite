<?php
	 ...
	 *
	 * - Creates & maintains the `wp_haru_termmeta` table
	 * - Listens for new blogs
	 * - Listens for blog deletion and deletes the `wp_haru_termmeta` table
	 * - Uses `WP_Meta_Query` and looks for    `meta_query` arguments
	 * - Deletes all metadata for terms when terms are deleted
	 * - Adds `wp_haru_termmeta` to the main database object when appropriate
	 * -
	 *
	...
	if ( ! class_exists( 'Haru_Term_Meta_Install' ) ):

		final class Haru_Term_Meta_Install {
			...
			public function __construct() {
				...
				// Setup plugin
				$this->file     = PLUGIN_HARU_PANGJA_CORE_FILE;
				$this->url      = PLUGIN_HARU_PANGJA_CORE_URL;
				$this->path     = PLUGIN_HARU_PANGJA_CORE_DIR;
				$this->basename = PLUGIN_HARU_PANGJA_CORE_NAME;
				$this->db       = $wpdb;
			...
			private function maybe_upgrade_database() {
				...
				$haru_term_meta_table_installed = (bool) get_option( 'haru_term_meta_table_installed', FALSE );
				// Needs
				if ( ! $haru_term_meta_table_installed ) {
					$this->activate();
					update_option( 'haru_term_meta_table_installed', TRUE );
				}
			}
			...
			public function add_termmeta_to_db_object() {
				global $wpdb;
				$this->db->haru_termmeta = $wpdb->haru_termmeta = "{$this->db->prefix}haru_termmeta";
			}
			...
			public function drop_tables( $tables = array() ) {
				...
				// Table to check for
				$table = "{$this->db->prefix}haru_termmeta";
			...
			/**
			 * Create the `haru_termmeta` table
			 *
			 * @since 0.1.0
			 */
			private function create_termmeta_table() {
				...
				dbDelta( array(
					        "CREATE TABLE {$this->db->prefix}haru_termmeta (
								meta_id bigint(20) NOT NULL AUTO_INCREMENT,
								term_id bigint(20) NOT NULL,
								haru_term_id bigint(20) NOT NULL,
			...
			public function delete_all_meta_for_term( $term_id = 0 ) {
				...
				$query         = "SELECT meta_id FROM {$this->db->haru_termmeta} WHERE term_id = %d";
				...
				foreach ( $term_meta_ids as $mid ) {
					delete_metadata_by_mid( 'haru_term', $mid );
				}
			}
		}
	endif; // class_exists( 'Haru_Term_Meta_Install' )

	if ( ! function_exists( 'Haru_Term_Meta_Install' ) ):
		 ...
		 * @since 0.1.0
		 */
		function Haru_Term_Meta_Install() {
			new Haru_Term_Meta_Install();
		}

		add_action( 'plugins_loaded', 'Haru_Term_Meta_Install' );

	endif; // function_exists( 'Haru_Term_Meta_Install' )