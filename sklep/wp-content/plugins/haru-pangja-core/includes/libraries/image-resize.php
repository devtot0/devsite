...
function haru_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {

	global $wpdb;

	if ( empty( $url ) )
		return new WP_Error( 'no_image_url', esc_html__( 'No image URL has been entered.','haru-pangja' ), $url );
	...
	// Check for Multisite
	// https://github.com/EricMcRay/Wordpress-Timthumb-alternative/commit/67e90d0aec2f311b334305731d96a368130d9575
	if ( is_multisite() ) {
		global $blog_id;
		$blog_details = get_blog_details( $blog_id );
		$file_path = str_replace( $blog_details->path, '/', $file_path );
		$url = str_replace( $blog_details->path, '/', $url );
	}
	...
	$name = wp_basename( $file_path, ".$ext" );
        ...
        if ( 'bmp' == $ext ) {
		return new WP_Error( 'bmp_mime_type', esc_html__( 'Image is BMP. Please use either JPG or PNG.','haru-pangja' ), $url );
	}
...
function haru_image_resize_id($images_id,$width = NULL, $height = NULL, $crop = true, $retina = false) {
    $output = '';
    $image_src = wp_get_attachment_image_src( $images_id, 'full' );
    if(count($image_src)>0) {
        $resize = haru_image_resize($image_src[0],$width,$height,$crop,$retina);
 ...
 */
add_action( 'delete_attachment', 'haru_delete_resized_images' );
function haru_delete_resized_images( $post_id ) {
