 ...
 */
 class Haru_WC_Widget_Price_Filter extends Haru_Widget {

	/**
	 * Constructor
	 */
	public function __construct() {
		$this->widget_cssclass    = 'haru_widget haru_widget_price_filter woocommerce widget_price_filter';
		$this->widget_description = esc_html__( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'haru-pangja' );
		$this->widget_id          = 'haru_woocommerce_price_filter';
		$this->widget_name        = esc_html__( 'Haru WooCommerce Price Filter List', 'haru-pangja' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by price', 'haru-pangja' ),
			...
			'price_range_size' => array(
				...
				'max'   => '',
				'std'   => 50,
				'label' => esc_html__( 'Price range size', 'haru-pangja' )
			),
			'max_price_ranges' => array(
				...
				'max'   => '',
				'std'   => 10,
				'label' => esc_html__( 'Max price ranges', 'haru-pangja' )
			),
			'hide_empty_ranges' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__( 'Hide empty price ranges', 'haru-pangja' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $max_unfiltered != $max ) {
            ...
            $range_size = apply_filters( 'haru_price_filter_range_size', $range_size ); // Requested: Make range-size filterable (can be useful when prices vary)
        } else {
          $range_size = intval( $instance['price_range_size'] );
        }
        $max_ranges = ( intval( $instance['max_price_ranges'] ) - 1 );
		
		$output = '<ul class="haru-price-filter">';
		        
		if ( strlen( $min_price ) > 0 ) {
			$output .= '<li><a href="' . esc_url( $link ) . '">' . esc_html__( 'All', 'haru-pangja' ) . '</a></li>';
		} else {
            $output .= '<li class="current">' . esc_html__( 'All', 'haru-pangja' ) . '</li>';
		}
...
if ( ! function_exists('haru_register_product_price_filter') ) {
	function haru_register_product_price_filter() {
		register_widget('Haru_WC_Widget_Price_Filter');
	}

	add_action('widgets_init', 'haru_register_product_price_filter', 1);
}