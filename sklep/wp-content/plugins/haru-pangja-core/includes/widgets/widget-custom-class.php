...
// Add custom CSS to Widget form
function haru_custom_css_in_widget_form( $widget, $return, $instance ) {
    if ( !isset( $instance['css_class'] ) ) $instance['css_class'] = null;
    echo "<p><label for='widget-{$widget->id_base}-{$widget->number}-css_class'>". esc_html__( 'CSS Class', 'haru-pangja' ) .":</label>
...
function haru_custom_css_widget_update_callback( $instance, $new_instance ) {
    $instance['css_class'] = array_key_exists('css_class',$new_instance) ? $new_instance['css_class'] : '';
    $instance['ids']       = array_key_exists('ids',$new_instance) ? $new_instance['ids'] : '';

    return $instance;
}

// Add Custom CSS Class to front end
function haru_custom_css_add_widget_css_class( $params ) {
...
// Front end hook
function haru_custom_css_frontend_hook() {
    if ( !is_admin() ) {
        add_filter( 'dynamic_sidebar_params','haru_custom_css_add_widget_css_class' );
    }
}

add_action( 'in_widget_form', 'haru_custom_css_in_widget_form', 10, 3 );
add_filter( 'widget_update_callback', 'haru_custom_css_widget_update_callback', 10, 2 );
add_action( 'wp_loaded', 'haru_custom_css_frontend_hook' );