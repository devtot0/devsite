...
class Haru_WC_Widget_Product_Search extends Haru_Widget {
	...
	public function __construct() {
		...
		$this->widget_description	= __( 'Display a product sorting list.', 'haru-pangja' );
		$this->widget_id          	= 'haru_woocommerce_widget_product_search';
		$this->widget_name        	= __( 'Haru Product Search', 'haru-pangja' );
		$this->settings           	= array(
			'type'  => array(
                'type'    => 'select',
                'std'     => '',
                ...
                'options' => array(
					'standard' => esc_html__( 'Standard Search', 'haru-pangja' ),
					'ajax'     => esc_html__( 'Ajax Search', 'haru-pangja' )
                )
            ),
		);
	...
	public function widget( $args, $instance ) {
		...
		$output .= 	'<div class="search-box-wrapper header-customize-item" data-hint-message="'.esc_attr__( "Please type at least 3 characters to search", "haru-pangja" ).'">
						<form method="get" action="'.esc_url(site_url()).'" class="search-type-'.esc_attr($search_box_type).' search-box">
							<input type="text" name="s" placeholder="'.esc_attr__( 'Search', 'haru-pangja' ).'"/>
