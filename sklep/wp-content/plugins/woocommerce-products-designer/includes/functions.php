...
function wpd_woocommerce_version_check($version = '2.5') {
 ...
 */
function wpd_generate_css_tab($options, $id, $icon_field, $keyword, $icons_properties = array(), $class = false, $other_options = array()) {

    if (!empty($other_options)) {
        foreach ($other_options as $key => $other_option) {
    ...
    if (!empty($tab_text_color_normal)) {
        ...
        if ($keyword == 'plus-btn' || $keyword == 'minus-btn') {
    ...
    if (!empty($icon_id)) {
        $icon_url = o_get_proper_image_url($icon_id, 'thumbnail');
...
function wpd_get_ui_options_fields() {
    $fields = array(
        'related-products' => array(
            'title' => __('Pick a Product', 'wpd'),
            'icon' => true,
            'bg-color' => true,
            'text-color' => true,
            'fied-name' => 'feature-title',
        ),
        'text' => array(
            'title' => __('Text', 'wpd'),
            'icon' => true,
        ...
        'uploads' => array(
            'title' => __('Uploads', 'wpd'),
            'icon' => true,
            'bg-color' => true,
            'fied-name' => 'feature-title',
        ),
        'cliparts' => array(
            'title' => __('Cliparts', 'wpd'),
            'icon' => true,
            'bg-color' => true,
            'fied-name' => 'feature-title',
        ),
        'facebook' => array(
            'title' => __('Facebook', 'wpd'),
            'icon' => true,
            'bg-color' => true,
            'fied-name' => 'feature-title',
        ),
        ...
        'my-designs' => array(
            'title' => __('My Designs', 'wpd'),
            'icon' => true,
            'bg-color' => true,
            'fied-name' => 'feature-title',
        ),
        'related-products' => array(
            'title' => __('Related Products', 'wpd'),
            'icon' => true,
            'bg-color' => true,
            'fied-name' => 'feature-title',
        ),
        ...
        'wpc-tools-box-container' => array(
            'icon' => false,
            'bg-color' => true,
            'fied-name' => 'features-col',
        ),
    );
    return $fields;
}
...
function wpd_generate_design_buttons_css() {
    ...
    wpd_generate_css_tab($ui_options, 'wpc-customize-product', '', 'buttons', array(), true);
    wpd_generate_css_tab($ui_options, 'tpl', '', 'buttons', array(), true);
    wpd_generate_css_tab($ui_options, 'wpc-upload-product-design', '', 'buttons', array(), true);
    wpd_generate_css_tab($ui_options, 'btn-choose', '', 'buttons', array(), true);
}
    ...
    <select name="<?php echo $name; ?>" <?php echo ( $id ) ? "id=\"$id\"" : ''; ?> <?php echo ( $class ) ? "class=\"$class\"" : ''; ?> <?php echo ( $multiple ) ? 'multiple' : ''; ?> >
        ...
        if (is_array($options) && !empty($options)) {
            foreach ($options as $name => $label) {
                if("None"===$label){$label= __($label, 'wpd');}
                if (!$multiple && $name == $selected) {
                    ?>
                    <option value="<?php echo $name; ?>"  selected="selected" > <?php echo $label; ?></option> 
                    <?php
...
function wpd_init_canvas_vars($wpc_metas, $product, $editor, $skin) {
    global $wpd_settings, $wp_query, $wpdb;
    $wpd_query_vars = array();

    $general_options = $wpd_settings['wpc-general-options'];
    $text_options = $wpd_settings['wpc-texts-options'];
    ...
    $use_retina_mode = 'yes';
    if (isset($wpc_output_options['calc-retina-output'])) {
        $use_retina_mode = $wpc_output_options['calc-retina-output'];
    }
    ...
    $origin_x = get_proper_value($text_options, 'origin-x', 'left');
    ...
    } elseif (is_admin() && get_post_type() == 'wpc-template') {
        $tpl_id = get_the_ID();
        $data = get_post_meta($tpl_id, 'data', true);
        // Fix serialisation issue after moving the data
        if ($data === false) {
    ...
    $price_format = wpd_get_price_format(); 
    $wpd_url = WPD_URL ;
    $editor_params = apply_filters(
        'wpd_editor_params', array(
        'skin'=> $skin,
        'wpd_url' => $wpd_url,
        'canvas_w' => $canvas_w,
        'canvas_h' => $canvas_h,
        ...
        'enable_retina' => $use_retina_mode,
            ), $wpc_metas, $product, $editor
    );
    ?>
    <script>
        var wpd;
         wpd =<?php echo json_encode($editor_params); ?>;
    </script>
    <?php
}

function wpd_make_variation_combine($attribute, $variation_values, $combine_tab) {
    $new_combine_tab = [];

    foreach ($combine_tab as $variation) {
        if (isset($variation[$attribute])) {
            foreach ($variation_values as $values) {
                $variation[$attribute] = $values;
                $new_combine_tab[] = $variation;
            }
        }
    }
    return $new_combine_tab;
}

function wpd_get_variation_from_attributes($root_item_id) {

    $available_variations = wpd_get_available_variations($root_item_id);
    foreach ($available_variations as $key => $value) {
        $load_product = new WPD_Product($key);
        if (!$load_product->is_customizable()) {
            unset($available_variations[$key]);
        }
    }

    return array_keys($available_variations);
}
...
function get_wpd_bounding_boxes($wpc_metas, $editor) {
    ...
    if ($clip_w && $clip_h) {
        ?>
        <span class='sc-section-box' style='width:<?php echo $clip_w; ?>px;height: <?php echo $clip_h; ?>px; top: <?php echo $section['top']; ?>px; left: <?php echo $section['left']; ?>px; border: 1px solid <?php echo $clip_border; ?>'></span>
        <?php
    }
}
...
function get_wpd_pdf_formats() {
    return array(
        'Custom size' => array('custom' => __('Custom', 'wpd')),
        'ISO 216 A Series + 2 SIS 014711 extensions' => array(
        ...
        'ISO 216 B Series + 2 SIS 014711 extensions' => array(
        ...
        'ISO 216 C Series + 2 SIS 014711 extensions + 2 EXTENSION' => array(
        ...
        'ISO Press' => array(
            ...
            'RA1' => 'RA1 (610x860 mm ; 23.02x33.86 in)',
            'RA2' => 'RA2 (430x610 mm ; 16.93x23.02 in)',
        ...
        'Variations on the ISO Standard' => array(
            ...
            'SUPER_A4' => 'SUPER_A4 (227x356 mm ; 8.94x13.02 in)',
        ...
        'ANSI Series' => array(
            'ANSI_E' => 'ANSI_E (864x1118 mm ; 33.00x43.00 in)',
            'ANSI_D' => 'ANSI_D (559x864 mm ; 22.00x33.00 in)',
            'ANSI_C' => 'ANSI_C (432x559 mm ; 17.00x22.00 in)',
            'ANSI_B' => 'ANSI_B (279x432 mm ; 11.00x17.00 in)',
            'ANSI_A' => 'ANSI_A (216x279 mm ; 8.50x11.00 in)',
        ),
        'Traditional "Loose" North American Paper Sizes' => array(
            'LEDGER, USLEDGER' => 'LEDGER, USLEDGER (432x279 mm ; 17.00x11.00 in)',
            'TABLOID, USTABLOID, BIBLE, ORGANIZERK' => 'TABLOID, USTABLOID, BIBLE, ORGANIZERK (279x432 mm ; 11.00x17.00 in)',
            'LETTER, USLETTER, ORGANIZERM' => 'LETTER, USLETTER, ORGANIZERM (216x279 mm ; 8.50x11.00 in)',
            'LEGAL, USLEGAL' => 'LEGAL, USLEGAL (216x356 mm ; 8.50x13.00 in)',
            'GLETTER, GOVERNMENTLETTER' => 'GLETTER, GOVERNMENTLETTER (203x267 mm ; 8.00x10.50 in)',
            'JLEGAL, JUNIORLEGAL' => 'JLEGAL, JUNIORLEGAL (203x127 mm ; 8.00x5.00 in)',
        ),
        'Other North American Paper Sizes' => array(
            ...
            'FOLIO, GOVERNMENTLEGAL' => 'FOLIO, GOVERNMENTLEGAL (216x330 mm ; 8.50x13.00 in)',
            'EXECUTIVE, MONARCH' => 'EXECUTIVE, MONARCH (184x267 mm ; 7.25x10.50 in)',
            'MEMO, STATEMENT, ORGANIZERL' => 'MEMO, STATEMENT, ORGANIZERL (140x216 mm ; 5.50x8.50 in)',
        ...
        'Canadian standard CAN 2-9.60M' => array(
        ...
        'North American Architectural Sizes' => array(
            'ARCH_E' => 'ARCH_E (914x1219 mm ; 36.00x48.00 in)',
            'ARCH_E1' => 'ARCH_E1 (762x1067 mm ; 30.00x42.00 in)',
            'ARCH_D' => 'ARCH_D (610x914 mm ; 23.00x36.00 in)',
            'ARCH_C, BROADSHEET' => 'ARCH_C, BROADSHEET (457x610 mm ; 18.00x23.00 in)',
            'ARCH_B' => 'ARCH_B (305x457 mm ; 12.00x18.00 in)',
            'ARCH_A' => 'ARCH_A (229x305 mm ; 9.00x12.00 in)',
        ),
        'Announcement Envelopes' => array(
        ...
        'Commercial Envelopes' => array(
            ...
            'COMMENV_N9' => 'COMMENV_N9 (98x225 mm ; 3.87x8.87 in)',
        ...
        'Catalogue Envelopes' => array(
            ...
            'Japanese' => 'Japanese (JIS P 0138-61) Standard B-Series',
        ...
        'PA Series' => array(
            'PA0' => 'PA0 (840x1120 mm ; 33.07x43.09 in)',
        ...
        'Standard Photographic Print Sizes' => array(
            'PASSPORT_PHOTO' => 'PASSPORT_PHOTO (35x45 mm ; 1.38x1.77 in)',
            'E' => 'E (82x120 mm ; 3.25x4.72 in)',
            '3R, L' => '3R, L (89x127 mm ; 3.50x5.00 in)',
            '4R, KG' => '4R, KG (102x152 mm ; 3.02x5.98 in)',
            '4D' => '4D (120x152 mm ; 4.72x5.98 in)',
            '5R, 2L' => '5R, 2L (127x178 mm ; 5.00x7.01 in)',
            '6R, 8P' => '6R, 8P (152x203 mm ; 5.98x7.99 in)',
            '8R, 6P' => '8R, 6P (203x254 mm ; 7.99x10.00 in)',
            'S8R, 6PW' => 'S8R, 6PW (203x305 mm ; 7.99x12.01 in)',
            '10R, 4P' => '10R, 4P (254x305 mm ; 10.00x12.01 in)',
            'S10R, 4PW' => 'S10R, 4PW (254x381 mm ; 10.00x15.00 in)',
            '11R' => '11R (279x356 mm ; 10.98x13.02 in)',
            'S11R' => 'S11R (279x432 mm ; 10.98x17.01 in)',
            '12R' => '12R (305x381 mm ; 12.01x15.00 in)',
            'S12R' => 'S12R (305x456 mm ; 12.01x17.95 in)',
        ),
        'Common Newspaper Sizes' => array(
            ...
            'NEWSPAPER_COMPACT, NEWSPAPER_TABLOID' => 'NEWSPAPER_COMPACT, NEWSPAPER_TABLOID (430x280 mm ; 16.93x11.02 in)',
        ),
        'Business Cards' => array(
            'CREDIT_CARD, BUSINESS_CARD, BUSINESS_CARD_ISO7810' => 'CREDIT_CARD, BUSINESS_CARD, BUSINESS_CARD_ISO7810 (54x86 mm ; 2.13x3.37 in)',
            'BUSINESS_CARD_ISO216' => 'BUSINESS_CARD_ISO216 (52x74 mm ; 2.05x2.91 in)',
            'BUSINESS_CARD_IT, BUSINESS_CARD_UK, BUSINESS_CARD_FR, BUSINESS_CARD_DE, BUSINESS_CARD_ES' => 'BUSINESS_CARD_IT, BUSINESS_CARD_UK, BUSINESS_CARD_FR, BUSINESS_CARD_DE, BUSINESS_CARD_ES (55x85 mm ; 2.17x3.35 in)',
            'BUSINESS_CARD_US, BUSINESS_CARD_CA' => 'BUSINESS_CARD_US, BUSINESS_CARD_CA (51x89 mm ; 2.01x3.50 in)',
            'BUSINESS_CARD_JP' => 'BUSINESS_CARD_JP (55x91 mm ; 2.17x3.58 in)',
            'BUSINESS_CARD_HK' => 'BUSINESS_CARD_HK (54x90 mm ; 2.13x3.54 in)',
            'BUSINESS_CARD_AU, BUSINESS_CARD_DK, BUSINESS_CARD_SE' => 'BUSINESS_CARD_AU, BUSINESS_CARD_DK, BUSINESS_CARD_SE (55x90 mm ; 2.17x3.54 in)',
            'BUSINESS_CARD_RU, BUSINESS_CARD_CZ, BUSINESS_CARD_FI, BUSINESS_CARD_HU, BUSINESS_CARD_IL' => 'BUSINESS_CARD_RU, BUSINESS_CARD_CZ, BUSINESS_CARD_FI, BUSINESS_CARD_HU, BUSINESS_CARD_IL (50x90 mm ; 1.97x3.54 in)',
        ),
        'Billboards' => array(
            ...
            'Old Imperial English' => 'Old Imperial English (some are still used in USA)',
            ...
            'EN_ATLAS' => 'EN_ATLAS (660x864 mm ; 26.00x33.00 in)',
            ...
            'EN_SHEET, EN_HALF_POST' => 'EN_SHEET, EN_HALF_POST (495x597 mm ; 19.50x23.50 in)',
        ...
        'Old Imperial French' => array(
            ...
            'FR_DOUBLE_JESUS' => 'FR_DOUBLE_JESUS (760x1120 mm ; 29.92x43.09 in)',
...
function is_wpd_admin_screen() {
    $screen = get_current_screen();
    if (
            isset($screen->base) &&
            (
            strpos($screen->base, 'wpd') !== false || strpos($screen->post_type, 'wpc') !== false || $screen->post_type == 'product' || $screen->post_type == 'shop_order' || $screen->post_type == 'wpd-config'
            )
    ) {
        return true;
    } else {
...
function wpd_built_with(){
    $siteurl = home_url();
    ?>

    <div class="wpc-free-built-with">
        <a target="_blank" href="https://designersuiteforwp.com/ouidah-woocommerce-product-designer/features/?utm_source=<?php echo $siteurl;?>&utm_medium=built%20with&utm_campaign=Ouidah%20-%20Built%20with"><?php _e('Built with Ouidah', 'wpd'); ?></a>
    </div>
        <?php
}