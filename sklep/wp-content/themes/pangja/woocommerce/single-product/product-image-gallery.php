...
if ( !in_array($single_product_thumbnail_columns, array('2', '3', '4', '5')) ) {
    $single_product_thumbnail_columns = haru_get_option('haru_single_product_thumbnail_columns');
}
// Set default
...
<div class="single-product-image-inner">
    <div id="product-images">
        <?php
            foreach($product_images as $key => $value) {
                ...
                echo sprintf( '<div id="product-image-%s" class="woocommerce-image-zoom">', $image_id );
                if ( !empty($variation_id) ) {
                    echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a href="%s" itemprop="image" class="woocommerce-main-image" title="%s" data-rel="prettyPhoto' . $gallery . '" data-variation_id="%s" data-index="%s"><i class="icon-magnifier"></i></a>%s', $image_link, $image_caption, $variation_id, $index, $image ), $post->ID );
                } else {
                    echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a href="%s" itemprop="image" class="woocommerce-main-image" title="%s" data-rel="prettyPhoto' . $gallery . '" data-index="%s"><i class="icon-magnifier"></i></a>%s', $image_link, $image_caption, $index, $image ), $post->ID );
                }
                echo '</div>';
            }
        ?>
    ...
    ?>
        <p class="product-video"><a href="<?php echo esc_url($haru_product_video_url); ?>" class="product-video-link" title="<?php echo esc_attr__( 'View Video', 'pangja' ); ?>" ><i class="ion ion-ios-play"></i><?php echo esc_html__( 'Video', 'pangja' ); ?></a></p>
    <?php endif; ?>
