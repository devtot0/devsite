...
class Haru_MegaMenu_Walker extends Walker_Nav_Menu {
    ...
    public function start_lvl( &$output, $depth = 0, $args = array() ) {
        // Menu item animate
        $menu_animation  = haru_get_option('haru_menu_animation');
        // Set Default
        if ( empty($menu_animation) ) {
            $menu_animation = 'menu_fadeInDown';
        }
        $sub_menu_animate = 'animated ' . $menu_animation;
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        // Do not display description with Post Type Archive menu item
        $description = (! empty( $item->description ) && ( $item->type_label != 'Post Type Archive' )) ? '<span>'.esc_attr( $item->description ).'</span>' : '';
