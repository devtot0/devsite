...
$css_classes[] = haru_get_css_animation($css_animation);
$animation_time = haru_get_style_animation($duration, $delay);

$css_class = preg_replace( '/\s+/', ' ', apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, implode( ' ', array_filter( array_unique( $css_classes ) ) ), $this->settings['base'], $atts ) );
// Custom add overlay color to row
if ( $overlay_set != 'hide_overlay' ) {
    $css_class .= ' overlay-bg-vc-wapper';
    if ( $overlay_set == 'show_overlay_color' ) {
        $overlay_color        = haru_convert_hex_to_rgba(esc_attr($overlay_color), (esc_attr($overlay_opacity) / 100));
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . $animation_time . '>';
