...
if (!have_posts()) {
    ...
    $page_title = single_tag_title(esc_html__( 'Tags: ', 'pangja' ), false);
} elseif (is_author()) {
    $page_title = sprintf(esc_html__( 'Author: %s', 'pangja' ), get_the_author());
} elseif (is_day()) {
    $page_title = sprintf(esc_html__( 'Daily Archives: %s', 'pangja' ), get_the_date());
} elseif (is_month()) {
    $page_title = sprintf(esc_html__( 'Monthly Archives: %s', 'pangja' ), get_the_date(_x('F Y', 'monthly archives date format', 'pangja')));
} elseif (is_year()) {
    $page_title = sprintf(esc_html__( 'Yearly Archives: %s', 'pangja' ), get_the_date(_x('Y', 'yearly archives date format', 'pangja')));
} elseif (is_search()) {
    $page_title = sprintf(esc_html__( 'Search Results for: %s', 'pangja' ), get_search_query());
} elseif (is_tax('post_format', 'post-format-aside')) {
    $page_title = esc_html__( 'Asides', 'pangja' );
} elseif (is_tax('post_format', 'post-format-gallery')) {
    $page_title = esc_html__( 'Galleries', 'pangja' );
} elseif (is_tax('post_format', 'post-format-image')) {
    $page_title = esc_html__( 'Images', 'pangja' );
} elseif (is_tax('post_format', 'post-format-video')) {
    $page_title = esc_html__( 'Videos', 'pangja' );
} elseif (is_tax('post_format', 'post-format-quote')) {
    $page_title = esc_html__( 'Quotes', 'pangja' );
} elseif (is_tax('post_format', 'post-format-link')) {
    $page_title = esc_html__( 'Links', 'pangja' );
} elseif (is_tax('post_format', 'post-format-status')) {
    $page_title = esc_html__( 'Statuses', 'pangja' );
} elseif (is_tax('post_format', 'post-format-audio')) {
    $page_title = esc_html__( 'Audios', 'pangja' );
} elseif (is_tax('post_format', 'post-format-chat')) {
    $page_title = esc_html__( 'Chats', 'pangja' );
// Custom Post type
} elseif (is_archive('haru_department')) {
    $page_title = esc_html__( 'Department', 'pangja' );
} else {
    $page_title = esc_html__( 'Archives', 'pangja' );
}
